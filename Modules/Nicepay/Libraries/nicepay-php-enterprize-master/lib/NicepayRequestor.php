...
class NicepayRequestor {
    ...
    public function operation($type) {
        ...
        if ($type == "requestVA") {
            $this->apiUrl = NICEPAY_V1_ENTERPRISE_REQ_VA_URL;
        } else if ($type == "threeDSecure") {
            $this->apiUrl = NICEPAY_V1_ENTERPRISE_3DSECURE_URL;
        } else if ($type == "chargeCard") {
            $this->apiUrl = NICEPAY_V1_ENTERPRISE_CHARGE_URL;
        } else if ($type == "cancel") {
            $this->apiUrl = NICEPAY_V1_ENTERPRISE_CANCEL_URL;
        } else if ($type == "checkPaymentStatus") {
            $this->apiUrl = NICEPAY_V1_ENTERPRISE_ORDER_STATUS_URL;
        } else if ($type == "cancelVA") {
            $this->apiUrl = NICEPAY_V1_ENTERPRISE_CANCEL_VA_URL;
        } else if ($type == "recurringToken") {
        $this->apiUrl = NICEPAY_V1_ENTERPRISE_RECURRING_TOKEN;
        }
    }
    ...
    public function openSocket() {
        ...
        $tryCount = 0;
        if (! $this->sock = @fsockopen ("ssl://".$host, $this->port, $errno, $errstr, NICEPAY_V1_ENTERPRISE_TIMEOUT_CONNECT )) {
            while ($tryCount < 12) {
                if ($this->sock = @fsockopen("ssl://".$host, $this->port, $errno, $errstr, NICEPAY_V1_ENTERPRISE_TIMEOUT_CONNECT )) {
                    return true;
                }
    ...
    public function apiRequest($data) {
        ...
        if($this->sock) {
            ...
            while ( ! feof ($this->sock ) && ! $timeout) {
                ...
                if ($diff >= NICEPAY_V1_ENTERPRISE_TIMEOUT_READ) {
                    $timeout = true;
                }
                if ($IsHeader) {
            ...
            if ($timeout) {
                $this->errorcode = NICEPAY_V1_ENTERPRISE_READ_TIMEOUT_ERR;
